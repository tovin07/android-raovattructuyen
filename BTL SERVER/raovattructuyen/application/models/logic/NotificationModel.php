...
class NotificationModel {
    ...
    public static function getById($notification_id) {
        ...
        $st = $conn->prepare($sql);
        $st->bindValue(1, $notification_id);
        $st->execute();
        $row = $st->fetch();
        $conn = null;
        return $row;
    ...
    public static function insert(Notification $notification) {
        $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8"));
        $sql = "INSERT INTO tbl_notification(post_id, user_id) 
            VALUES (?, ?)";
        $st = $conn->prepare($sql);
        $st->bindValue(1, $notification->getPost_id());
        $st->bindValue(2, $notification->getUser_id());
        $result = $st->execute();
        $conn = null;
        return $result;
    }
