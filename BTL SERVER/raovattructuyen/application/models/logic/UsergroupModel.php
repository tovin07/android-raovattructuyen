...
class UsergroupModel {
    ...
    public static function getByUsergroupname($usergroup_name) {
        ...
        $st = $conn->prepare($sql);
        $st->bindValue(1, $usergroup_usergroupname);
        $st->execute();
        $row = $st->fetch();
        $conn = null;
        return $row;
    ...
    public static function insert(Usergroup $usergroup) {
        ...
        $sql = "INSERT INTO tbl_usergroup(usergroup_name, usergroup_description) 
            VALUES (?, ?)";
        $st = $conn->prepare($sql);
        $st->bindValue(1, $usergroup->usergroup_name);
        $st->bindValue(2, $usergroup->usergroup_description);
        $result = $st->execute();
        $conn = null;
        return $result;
    }
    ...
    public static function update($usergroup_id, Usergroup $usergroup) {
        ...
        $sql = "UPDATE tbl_usergroup SET usergroup_name= ?, usergroup_description= ?
             WHERE usergroup_id= ?";
        $st = $conn->prepare($sql);
        $st->bindValue(1, $usergroup->getUsergroup_name());
        $st->bindValue(2, $usergroup->getUsergroup_description());
        $st->bindValue(3, $usergroup_id);
        $st->execute();
        $conn = null;
    }
    /*
